...
- After finishing work, run `pnpm check` to typecheck, format, and lint the entire project. Fix any errors before committing.
- Before building a custom component, assess whether Mantine already provides a suitable component or composition of components. Always prioritize Mantine's premade components over custom implementations.
...
Follow the [TanStack ESLint import rules](https://github.com/TanStack/config/blob/main/packages/eslint-config/src/import.ts). Imports must be ordered by group, separated by empty lines between each group:
...
1. **Always prefer Mantine components** — use `<Button>`, `<TextInput>`, `<Stack>`, `<Group>`, `<Paper>`, `<Text>`, `<Title>`, etc. over raw HTML elements whenever possible. Before creating a custom component, check if Mantine already provides one or if a composition of existing components can achieve the same result.
